        ...
        if self.loop == False:
            try:
                ...
                with async_timeout.timeout(300):
                    ...
                    self.tmp = track.title
            except asyncio.TimeoutError:
            # No music has been played for 5 minutes, cleanup and disconnect...
                return await self.teardown()
        else:
            pretrack = self.tmp
            tracks = await self.bot.wavelink.get_tracks(f'ytsearch:{pretrack}')
            track = Track(tracks[0].id, tracks[0].info, requester=mention(mention="<@746821549918519417>"))
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def start_nodes(self) -> None:
        ...
        nodes = {'MAIN': {'host': '127.0.0.1',
                          'port': 2112,
                          'rest_uri': 'http://127.0.0.1:2112',
                          'password': 'aidghosty275',
                          'identifier': 'MAIN',
                          'region': 'us_central'
                          }}
    ...
    async def loop(self, ctx):
        ...
        if player.loop == True:
            ...
            player.loop = True
            await ctx.send("```Loop is now on.```")
    @commands.command(name="seek")
    async def _seek(self, ctx, string):
        tmp = string.split(":")
        player = self.bot.wavelink.get_player(guild_id=ctx.guild.id, cls=Player, context=ctx)
        await player.seek((((int(tmp[0])*60*60)+(int(tmp[1])*60)+(int(tmp[2])))*1000))
